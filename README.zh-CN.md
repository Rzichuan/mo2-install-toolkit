...
GitHub Release 中的 `mo2-runtime-v0.10.1-win-x64.zip` **不是** Skill/plugin，也不是独立安装程序。它只是 clone 后的 Skill/plugin 自动下载的可执行运行时依赖。普通用户应使用一键安装命令，不需要手动下载 Release 资产；安装器会自动使用 Skill 和 Runtime 两类资产。
...
最终元数据路径必须是 `%LOCALAPPDATA%\MO2AgentToolkit\runtimes\0.10.1\mo2-runtime\runtime.json`，不要产生 `mo2-runtime\mo2-runtime` 双层目录。
