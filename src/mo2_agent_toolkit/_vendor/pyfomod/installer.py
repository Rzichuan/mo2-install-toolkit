...
class Installer(object):
    ...
    def next(self, selected_options=None):
        if self._has_finished:
            ...
            for page in self._order_list(self.root.pages, self.root.pages.order):
                try:
    ...
    def file_infos(self):
        """Return resolved files without the upstream dict's duplicate-source loss."""
        ...
        return [FileInfo(source, destination, priority_dict[destination]) for destination, source in file_dict.items()]

    def files(self):
        return {info.source: info.destination for info in self.file_infos()}
    ...
    def _test_version_condition(self, version):
        ...
        game_version = version_key(self.game_version)
        version = version_key(version)
        width = max(len(game_version), len(version))
        game_version += (0,) * (width - len(game_version))
