...
def _parse_target(file_path, warnings=None):
...
def parse(source, warnings=None, lineno=False):
    if isinstance(source, (tuple, list)):
        ...
        info = str(info) if info.is_file() else None
        if not conf.is_file():
            raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), "moduleconfig.xml")
        conf = str(conf)
    # The upstream schema-warning pass depends on lxml. Runtime parsing and all
    # installer semantics use the same pyfomod Target model without that pass.
    try:
        root = _parse_target(conf, warnings)
        if info is not None:
            root._info = _parse_target(info, warnings)
