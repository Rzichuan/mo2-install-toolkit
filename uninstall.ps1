...
foreach($Path in $Validated){if((Test-RequestedAction $Path 'Remove managed adapter')){Remove-Junction $Path}}
if((Test-Path -LiteralPath $Bundle) -and (Test-RequestedAction $Bundle 'Remove managed Skill')){Remove-Item -LiteralPath $Bundle -Recurse -Force}
if($RemoveRuntime -and $Data.PSObject.Properties.Name -contains 'runtime'){$Runtime=[IO.Path]::GetFullPath([string]$Data.runtime);if(-not $Runtime.StartsWith($Root+'\',[StringComparison]::OrdinalIgnoreCase)){throw 'Recorded runtime is outside the toolkit data root.'};if((Test-Path -LiteralPath $Runtime) -and (Test-RequestedAction $Runtime 'Remove installed runtime')){Remove-Item -LiteralPath $Runtime -Recurse -Force}}
if((Test-RequestedAction $Manifest 'Remove installation manifest')){Remove-Item -LiteralPath $Manifest -Force}
