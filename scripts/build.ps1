...
$PyInstallerArguments = @(
  '-X', 'utf8', '-m', 'PyInstaller',
  '--noconfirm', '--clean', '--onedir', '--name', 'mo2-tool',
  '--paths', (Join-Path $Root 'src'),
  '--add-binary', "$Patcher;mo2_agent_toolkit\bin"
)
$LegacySource = Join-Path $Root 'src\mo2_agent_toolkit\legacy'
foreach ($LegacyScript in @(Get-ChildItem -LiteralPath $LegacySource -Filter '*.py' -File | Sort-Object Name)) {
  $PyInstallerArguments += @('--add-data', "$($LegacyScript.FullName);mo2_agent_toolkit\legacy")
}
$PyInstallerArguments += (Join-Path $Root 'scripts\entrypoint.py')
